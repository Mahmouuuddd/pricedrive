...
SIDEBAR_STYLE = {
    ...
    "backgroundColor": BG_INPUT, "color": TEXT_MAIN,
    "borderRight": f"1px solid {BORDER}", "zIndex": 100,
}
CONTENT_STYLE = {
    "marginLeft": "260px", "padding": "32px",
    "background": f"linear-gradient(160deg, {BG_PRIMARY} 0%, {BG_SECONDARY} 50%, {BG_INPUT} 100%)",
    "minHeight": "100vh", "color": TEXT_MAIN,
}
CARD_STYLE = {
    "backgroundColor": BG_CARD, "borderRadius": "14px",
    "boxShadow": "0 4px 18px rgba(0,0,0,0.20)",
    "border": f"1px solid {BORDER}", "padding": "20px", "marginBottom": "20px",
}
INPUT_STYLE = {
    "backgroundColor": BG_INPUT, "color": TEXT_MAIN,
    "border": f"1px solid {BORDER}", "borderRadius": "8px", "minHeight": "38px",
}
INPUT_STYLE_DROPDOWN = {
    "backgroundColor": BG_INPUT, "color": TEXT_MAIN2,
    "border": f"1px solid {BORDER}", "borderRadius": "8px", "minHeight": "38px",
}
RESULT_BOX_STYLE = {
    "borderRadius": "12px", "padding": "24px", "textAlign": "center",
    "backgroundColor": BG_INPUT, "border": f"2px dashed {BORDER}",
...
GLOBAL_INLINE_CSS = f"""
:root {{
    --pd-bg-primary: {BG_PRIMARY}; --pd-bg-secondary: {BG_SECONDARY};
    --pd-bg-card: {BG_CARD}; --pd-bg-input: {BG_INPUT};
    --pd-border: {BORDER}; --pd-text-main: {TEXT_MAIN};
    --pd-text-muted: {TEXT_MUTED}; --pd-accent: {ACCENT_SKY};
    --pd-accent-strong: {ACCENT_BLUE}; --pd-danger: {ACCENT_RED};
}}
.form-control,.form-select,input,textarea,select {{
    background-color:var(--pd-bg-input)!important; color:var(--pd-text-main)!important;
    border:1px solid var(--pd-border)!important; border-radius:8px!important; box-shadow:none!important;
}}
.form-control::placeholder,input::placeholder,textarea::placeholder {{
    color:var(--pd-text-muted)!important; opacity:1!important;
}}
.form-control:focus,.form-select:focus,input:focus,textarea:focus,select:focus {{
    background-color:var(--pd-bg-input)!important; color:var(--pd-text-main)!important;
    border-color:var(--pd-accent)!important; box-shadow:0 0 0 0.2rem rgba(88,166,255,0.15)!important;
}}
.Select-control,.Select-menu-outer,.Select-menu,.Select-value,.Select-placeholder,
.Select-input>input,.Select-arrow-zone,.Select-clear-zone,.Select-option,
.VirtualizedSelectOption,.VirtualizedSelectFocusedOption {{
    background-color:var(--pd-bg-input)!important; color:var(--pd-text-main)!important;
}}
.Select-control {{
    border:1px solid var(--pd-border)!important; border-radius:8px!important;
    min-height:38px!important; background-image:none!important;
}}
.is-focused:not(.is-open)>.Select-control,.is-open>.Select-control,.Select-control:hover {{
    border-color:var(--pd-accent)!important; box-shadow:0 0 0 0.2rem rgba(88,166,255,0.15)!important;
}}
.Select-placeholder {{ color:var(--pd-text-muted)!important; }}
.Select--single>.Select-control .Select-value,.Select-value-label,
.has-value.Select--single>.Select-control .Select-value .Select-value-label {{
    color:var(--pd-text-main)!important;
}}
.Select-input>input {{ color:var(--pd-text-main)!important; }}
.Select-menu-outer {{ border:1px solid var(--pd-border)!important; border-top:none!important; }}
.Select-option,.VirtualizedSelectOption {{ border-bottom:1px solid rgba(255,255,255,0.04)!important; }}
.Select-option.is-focused,.VirtualizedSelectFocusedOption {{
    background-color:#1f2a3a!important; color:var(--pd-text-main)!important;
}}
.Select-option.is-selected {{ background-color:var(--pd-accent-strong)!important; color:white!important; }}
.Select-arrow {{ border-top-color:var(--pd-text-muted)!important; }}
.is-open .Select-arrow {{ border-bottom-color:var(--pd-text-muted)!important; border-top-color:transparent!important; }}
.Select-clear-zone,.Select-arrow-zone {{ color:var(--pd-text-muted)!important; }}
input::-webkit-outer-spin-button,input::-webkit-inner-spin-button {{ filter:invert(0.85); }}
input[disabled],.form-control[disabled],.Select.is-disabled>.Select-control {{
    opacity:0.65!important; background-color:#111822!important;
}}
...
<html>
    <head>
        {{%metas%}}<title>PriceDrive</title>{{%favicon%}}{{%css%}}
...
def style_figure(fig):
    fig.update_layout(
        template=PLOT_TEMPLATE, paper_bgcolor=BG_CARD, plot_bgcolor=BG_CARD,
        font=dict(color=TEXT_MAIN), title_font=dict(size=18),
...
def overview_layout():
    ...
    fig_cond = style_figure(px.histogram(df, x='condition', nbins=50,
        title='Condition Rating Distribution', labels={'condition': 'Condition Score'},
        color_discrete_sequence=[ACCENT_SKY]))
    fig_cond.update_layout(bargap=0.05, height=320)

    fig_price = style_figure(px.histogram(df, x='sellingprice', nbins=80,
        title='Selling Price Distribution', labels={'sellingprice': 'Selling Price ($)'},
    ...
    fig_makes = style_figure(px.bar(top_makes, x='count', y='make', orientation='h',
        title='Top 10 Makes by Sales Volume',
        labels={'count': 'Number of Sales', 'make': 'Make'},
        color='count', color_continuous_scale='Blues'))
    fig_makes.update_layout(height=360, showlegend=False, coloraxis_showscale=False,
                            yaxis=dict(autorange='reversed'))

    sample = df.sample(n=min(15_000, len(df)), random_state=42)
    fig_scatter = style_figure(px.scatter(sample, x='odometer', y='sellingprice',
        color='vehicle_age', color_continuous_scale='Viridis_r', opacity=0.4,
        ...
        html.Div([
            dbc.Row([
                ...
                dbc.Col([
                    ...
                    dcc.Dropdown(id='trend-trans-filter',
...
def market_layout():
    ...
    fig_donut = style_figure(px.pie(body_counts, names='body', values='count',
    ...
    make_summary = (
        df.groupby('make').agg(count=('sellingprice','count'),
            avg_price=('sellingprice','mean'), avg_condition=('condition','mean'))
        .reset_index().query('count >= 200').sort_values('count', ascending=False).head(20)
    )
    fig_bubble = style_figure(px.scatter(make_summary, x='avg_price', y='count',
        size='count', color='avg_condition', text='make', color_continuous_scale='RdYlGn',
        title='Top Makes: Sales Volume vs Average Price',
        labels={'avg_price': 'Avg Selling Price ($)', 'count': 'Sales Volume',
    ...
    body_stats = (
        df[df['body'] != 'unknown'].groupby('body')
        .agg(count=('sellingprice','count'), avg_price=('sellingprice','mean'))
        .reset_index().sort_values('count', ascending=False).head(10)
    )
    fig_body = make_subplots(rows=1, cols=2,
        subplot_titles=('Volume by Body Type', 'Avg Price by Body Type'))
    fig_body.add_trace(go.Bar(x=body_stats['body'], y=body_stats['count'],
        marker_color=ACCENT_BLUE, name='Volume'), row=1, col=1)
    fig_body.add_trace(go.Bar(
        x=body_stats.sort_values('avg_price', ascending=False)['body'],
        y=body_stats.sort_values('avg_price', ascending=False)['avg_price'],
        marker_color=ACCENT_RED, name='Avg Price'), row=1, col=2)
    fig_body.update_layout(height=360, showlegend=False)
    fig_body.update_xaxes(tickangle=-35)
    style_figure(fig_body)

    df['condition_bin'] = pd.cut(df['condition'], bins=[0,10,20,30,40,50],
        labels=['1-10','11-20','21-30','31-40','41-50'])
    fig_box = style_figure(px.box(df, x='condition_bin', y='sellingprice',
        title='Selling Price by Condition Rating',
        labels={'condition_bin': 'Condition (binned)', 'sellingprice': 'Selling Price ($)'},
        color='condition_bin', color_discrete_sequence=px.colors.sequential.Viridis,
        category_orders={'condition_bin': ['1-10','11-20','21-30','31-40','41-50']}))
    fig_box.update_layout(showlegend=False, height=380, yaxis=dict(range=[0, 60000]))
        ...
        html.Div([
            dbc.Row([
                dbc.Col([
                    ...
                    dcc.Dropdown(id='pred-make',
                        options=[{'label': m.title(), 'value': m} for m in makes],
                        placeholder='Select make...', clearable=False, style=INPUT_STYLE_DROPDOWN),
                    ...
                    dcc.Dropdown(id='pred-model', placeholder='Select make first...',
                        clearable=False, style=INPUT_STYLE_DROPDOWN),
                    ...
                    dcc.Dropdown(id='pred-trim', placeholder='Select model first...',
                        clearable=True, style=INPUT_STYLE_DROPDOWN),
                    html.Br(),
                    html.Label("Body Type *", style={"fontWeight": "600", "fontSize": "13px", "color": TEXT_MAIN}),
                    dcc.Dropdown(id='pred-body',
                        options=[{'label': b.title(), 'value': b} for b in body_types],
                        placeholder='Select body type...', clearable=False, style=INPUT_STYLE_DROPDOWN),
                    ...
                    dcc.Dropdown(id='pred-transmission',
                        options=[{'label': t.title(), 'value': t} for t in transmissions],
                        placeholder='Select transmission...', clearable=False, style=INPUT_STYLE_DROPDOWN),
                ...
                dbc.Col([
                    ...
                    dbc.Input(id='pred-year', type='number', placeholder='e.g. 2014',
                              min=1982, max=2015, step=1, style=INPUT_STYLE),
                    ...
                    dbc.Input(id='pred-odometer', type='number', placeholder='e.g. 35000',
                              min=10, max=300000, step=1, style=INPUT_STYLE),
                    ...
                    dbc.Input(id='pred-condition', type='number', placeholder='e.g. 35',
                    ...
                    dbc.Input(id='pred-age', type='number', placeholder='e.g. 3',
                              min=0, max=33, step=1, style=INPUT_STYLE),
                    html.Div(id='err-age', style={"color": ACCENT_RED, "fontSize": "12px", "marginTop": "4px"}),
                    html.Br(),
                    html.P("* Required fields", style={"color": TEXT_MUTED, "fontSize": "12px"}),
                ], width=4),
                dbc.Col([
                    dbc.Button("Predict Price 🚗", id='pred-btn', color='primary', size='lg',
                        style={"width": "100%", "fontWeight": "700", "marginBottom": "20px",
                               "background": f"linear-gradient(135deg, {ACCENT_BLUE}, #1f6feb)",
                               "border": "none"}),
                    html.Div(id='pred-result', style=RESULT_BOX_STYLE),
                    html.Div(id='pred-note', style={"fontSize": "12px", "color": TEXT_MUTED, "textAlign": "center"}),
                ], width=4),
            ]),
        ], style=CARD_STYLE),
        html.Div([
...
def render_page(pathname):
    if pathname == '/trends':      return trends_layout()
    elif pathname == '/market':    return market_layout()
    elif pathname == '/predictor': return predictor_layout()
...
def update_trends(make_filter, body_filter, trans_filter):
    dff = df.copy()
    if make_filter  != 'all': dff = dff[dff['make']         == make_filter]
    if body_filter  != 'all': dff = dff[dff['body']         == body_filter]
    if trans_filter != 'all': dff = dff[dff['transmission'] == trans_filter]

    age_stats = (
        dff[dff['vehicle_age'] <= 20].groupby('vehicle_age')['sellingprice']
    ...
    fig1.add_trace(go.Scatter(x=age_stats['vehicle_age'], y=age_stats['mean'],
        mode='lines+markers', name='Mean price', line=dict(color=ACCENT_BLUE, width=2.5)))
    fig1.add_trace(go.Scatter(x=age_stats['vehicle_age'], y=age_stats['median'],
        mode='lines+markers', name='Median price',
        line=dict(color=ACCENT_RED, width=2.5, dash='dash')))
    fig1.update_layout(title='Price Depreciation by Vehicle Age',
        xaxis_title='Vehicle Age (years)', yaxis_title='Selling Price ($)',
        hovermode='x unified', height=360)
    style_figure(fig1)

    dff['condition_bin'] = pd.cut(dff['condition'], bins=[0,10,20,30,40,50],
        labels=['1-10','11-20','21-30','31-40','41-50'])
    cond_price = dff.groupby('condition_bin', observed=True)['sellingprice'].mean().reset_index()
    fig2 = style_figure(px.bar(cond_price, x='condition_bin', y='sellingprice',
        title='Average Selling Price by Condition Rating',
        labels={'condition_bin': 'Condition (binned)', 'sellingprice': 'Avg Selling Price ($)'},
    ...
    fig3 = style_figure(px.bar(year_vol, x='year', y='count',
...
def update_models(make):
    if not make: return []
    return [{'label': m.title(), 'value': m}
            for m in sorted(df[df['make'] == make]['model'].dropna().unique().tolist())]


@app.callback(Output('pred-trim', 'options'),
              Input('pred-make', 'value'), Input('pred-model', 'value'))
def update_trims(make, model_val):
    if not make or not model_val: return []
    return [{'label': t, 'value': t}
            for t in sorted(df[(df['make'] == make) & (df['model'] == model_val)]
...
def predict_price(n_clicks, make, model_val, trim, body,
    ...
    if not make:          err['make']         = '⚠ Make is required';          has_error = True
    if not model_val:     err['model']        = '⚠ Model is required';         has_error = True
    if not body:          err['body']         = '⚠ Body type is required';     has_error = True
    if not transmission:  err['transmission'] = '⚠ Transmission is required';  has_error = True
    if year is None:      err['year']         = '⚠ Year is required';          has_error = True
    elif not (1982 <= int(year) <= 2015):
                          err['year']         = '⚠ Must be 1982–2015';         has_error = True
    if odometer is None:  err['odometer']     = '⚠ Odometer is required';      has_error = True
    elif not (10 <= int(odometer) <= 300000):
                          err['odometer']     = '⚠ Must be 10–300,000 miles';  has_error = True
    if condition is None: err['condition']    = '⚠ Condition is required';     has_error = True
    elif not (1 <= int(condition) <= 50):
                          err['condition']    = '⚠ Must be 1–50';              has_error = True
    if age is None:       err['age']          = '⚠ Vehicle age is required';   has_error = True
    elif int(age) < 0:    err['age']          = '⚠ Cannot be negative';        has_error = True

    empty_fig = go.Figure()
    empty_fig.update_layout(template=PLOT_TEMPLATE, paper_bgcolor=BG_CARD,
        plot_bgcolor=BG_CARD, font=dict(color=TEXT_MAIN), height=300,
        annotations=[dict(text="Fill in the form and click Predict",
    ...
    trim_val = trim if trim else (
        df[(df['make'] == make) & (df['model'] == model_val)]['trim'].mode().iloc[0]
        if len(df[(df['make'] == make) & (df['model'] == model_val)]) > 0 else 'base'
    )

    row = {'year': int(year), 'make': make, 'model': model_val, 'trim': trim_val,
           'body': body, 'transmission': transmission, 'condition': int(condition),
           'odometer': int(odometer), 'vehicle_age': int(age)}
    ...
    predicted = max(500.0, float(model.predict(input_df)[0]))

    similar = df[(df['make'] == make) & (df['body'] == body) &
                 (df['vehicle_age'].between(int(age) - 1, int(age) + 1))]['sellingprice']
    note_text = (f"Similar {make.title()} {body}s aged ~{age} yrs typically sell "
                 f"between ${similar.quantile(0.25):,.0f} – ${similar.quantile(0.75):,.0f}"
                 if len(similar) >= 5
                 else "Not enough similar vehicles in dataset for market comparison.")
    ...
    fig_imp = style_figure(px.bar(feat_imp, x='importance', y='feature', orientation='h',
